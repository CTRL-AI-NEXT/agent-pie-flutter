import 'package:agent_pie/core/model/response_model/training_response_model.dart';
...
class ActionsController extends GetxController {
  ...
  final trainingResult = Rx(APIResult<TrainingResponseModel?>.loading());
  final RxList<Step> recipeSteps = <Step>[].obs;
  ...
  Future<void> callGetQuizAPI(
      {required int id}) async {
    trainingResult.value = APIResult<TrainingResponseModel>.loading();


    var networkResult = await APIHelper.instance.callGetApi(
      '${NetworkConstant.training}/${id.toString()}',
      null,
      true,
    );

    var apiResultFromNetwork = getAPIResultFromNetworkWithoutBase<TrainingResponseModel?>(networkResult,
            (e) => TrainingResponseModel.fromJson(e));

    trainingResult.value = apiResultFromNetwork;

    if (trainingResult.value.apiResultType != APIResultType.success) {
      AppUtils.showSnackBar(
          message: trainingResult.value.message ?? '',
          isSuccess: false);
    }else{
      recipeSteps.value = trainingResult.value.result!.steps!;
      recipeSteps.refresh();
      logger.i(trainingResult.value.result?.toJson());
      // quiz.value = Quiz.fromJson(quizAPIResult.value.result!.toJson());
    }
  }
}