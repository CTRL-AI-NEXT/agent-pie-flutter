...
class QuizController extends GetxController {
  ...
  final resultQuizResponse = Rx(APIResult<Quiz?>.loading());
  ...
  Future<void> loadQuestions() async {
    ...
    resultQuizResponse.value = APIResult<Quiz>.loading();

    var networkResult = await APIHelper.instance.callGetApi(
      '${NetworkConstant.quizzes}/1',
      {},
      false,
    );

    var apiResultFromNetwork = getAPIResultFromNetworkWithoutBase<Quiz?>(
        networkResult, (e) => Quiz.fromJson(e));

    resultQuizResponse.value = apiResultFromNetwork;
    if (resultQuizResponse.value.apiResultType == APIResultType.success) {
      if (resultQuizResponse.value.result != null) {
        quiz.value = resultQuizResponse.value.result!;
      } else {}
    } else {
      AppUtils.showSnackBar(message: 'Something went wrong', isSuccess: false);
    }
  }

  void selectOption(int questionId, String option) {
    final questionIndex =
        quiz.value?.questions.indexWhere((q) => q.id == questionId);
    ...
    if (question.questionType != 'open_ended' &&
        questionIndex < quiz.value!.questions.length - 1) {
