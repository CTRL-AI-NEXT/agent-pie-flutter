...
import 'package:agent_pie/core/network/api_response_handler.dart';
import 'package:agent_pie/core/network/api_result.dart';
...
class ChatsController extends GetxController {
  ...
  final resultChatResponse = Rx(APIResult<ChatMessageResponseModel?>.loading());
  ...
  void onInit() {
    ...
    messageList
        .sort((a, b) => b.createdAt!.compareTo(a.createdAt ?? DateTime(0)));
  ...
  // Call Get Response from Query API
  Future<ChatMessageResponseModel?> callGetResponseFromQueryAPI(
      {required String query}) async {
    try {
      isSendingMessage.value = true;
      resultChatResponse.value = APIResult<ChatMessageResponseModel>.loading();
      ...
      var apiResultFromNetwork =
          getAPIResultFromNetworkWithoutBase<ChatMessageResponseModel?>(
              networkResult, (e) => ChatMessageResponseModel.fromJson(e));

      resultChatResponse.value = apiResultFromNetwork;
      if (resultChatResponse.value.apiResultType == APIResultType.success) {
        if (resultChatResponse.value.result != null) {
          return resultChatResponse.value.result!;
        } else {
          return ChatMessageResponseModel(
            answer: 'No answer from server.',
            sopTitle: 'Ad-hoc Query',
        ...
        AppUtils.showSnackBar(
            message: resultChatResponse.value.message ?? 'Something went wrong',
            isSuccess: false);
      }
      return ChatMessageResponseModel(
        answer: '',
  ...
  Future<void> onMessageSend() async {
    ...
    final newChatItem = await callGetResponseFromQueryAPI(
        query: chatTextEditingController.text);

    if (newChatItem == null) return;
    messageList.insert(0, newChatItem);
